...
class SumkDFT(object):
    ...
    def __init__(self, hdf_file, h_field=0.0, use_dft_blocks=False,
                 ...
                 misc_data='dft_misc_input',bc_data='dft_bandchar_input',fs_data='dft_fs_input'): #adnj edit
        r"""
        ...
        if not type(hdf_file) == StringType:
            ...
            self.bc_data = bc_data #adnj
            self.fs_data = fs_data #adnj
...
################
    ...
    def calc_density_correction(self, filename=None, dm_type='wien2k', spinave=False): #adnj edit
        r"""
        ...
        assert dm_type in ('vasp', 'wien2k','elk'), "'dm_type' must be either 'vasp', 'wienk' or 'elk'" #adnj
...
# In 'vasp'-mode subtract the DFT density matrix
        ...
        # adnj edit - now save to Elk output file:
        elif dm_type == 'elk':
        # this is similar to the vasp implemented - but with SP
            if mpi.is_master_node():
...
# Triqs to Elk functions - adnj
################
    #just output the density matrix
#    def calc_density_correction_elk(self, filename=None, dm_type='elk'):
    #extra input for averaging over spin densities
    def calc_density_correction_elk(self, filename=None, dm_type='elk', spinave=False):
        r"""
        Calculates the charge density correction and stores it into a file.
        ...
        """
        assert dm_type in ('elk'), "'dm_type' must be 'elk'"

        #assert type(filename) == StringType, ("calc_density_correction_elk: "
        #                                      "filename has to be a string!")
