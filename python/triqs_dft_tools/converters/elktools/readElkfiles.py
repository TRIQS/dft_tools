...
class readElkfiles:
              ...
              if (basis[n_shells]==2):
                ...
                for i in range(n_orb):
                  for j in range(n_orb):
                    T[n_shells][i, j] = R.next()
                for i in range(n_orb):
                  for j in range(n_orb):
                    T[n_shells][i, j] += 1j * R.next()
    ...
    def determine_rep(self,ish,ishin,corr_shells,basis,ind,n_reps,dim_reps,irep):
      ...
      #determine the dim_reps from the lm indices in ind     
         ...
         raise "Elk_converter (determine_rep) : irreducible representations were not found!"
    ...
    def read_projector(self,shell,n_spin_blocks,ish,proj_mat,ind,T,basis,filext):
        ...
        #no. of kpts and number of orbital
          ...
          #k-point index and correlated band window indices
             ...
             for isp in range(0,n_spin_blocks):
               ...
               if(isp+1!=proj_dim['isp']):
                 raise "Elk_converter (",wan_shells_file,") : reading spin projecotrs failed!"
                 return
          ...
          proj_mat=proj_mat[:,:,:,:,:numpy.max(n_orbitals)]
            ...
            raise "Elk_converter (read projectors): reading file failed!"
        R.close()
    ...
    def read_eig(self,filext=".OUT"):
        ...
        #no. second variation states (no. bands)
          ...
          for ik in range(0,n_k):
            ...
            raise "Elk_converter (EIGVAL.OUT): reading file failed!"
        R.close()
        return(en,occ,nstsv)
    ...
    def read_kpoints(self,filext=".OUT"):
        ...
        #reads in the k index, lattice vectors, weights and nmat for each kpt
          ...
          #read in BZ weights
            ...
            raise "Elk_converter (KPOINTS.OUT): reading file failed!"
        R.close()
        ...
        for ish in range(n_shells):
          ...
          try:
            ...
            for isym in range(n_symm):
              ...
              perm.append([int(R.next()) for i in range(n_atoms)])
              n_orb=2*shells[ish]['l']+1
              #read in the lm symmetry matrix
              # real part
              for i in range(n_orb):
                for j in range(n_orb):
                  symmat[isym][ish][i, j] = R.next()
              # imaginary part
              for i in range(n_orb):
                for j in range(n_orb):
                  symmat[isym][ish][i, j] += 1j * R.next()
          except StopIteration:  # a more explicit error if the file is corrupted.
              raise "Elk_converter (",projsym_file,") : reading file failed!"
          R.close()
    ...
    def readsym(self):
        ...
        try:
          ...
          raise "Elk_converter : reading SYMCRYS.OUT file failed!"
        R.close()
      ...
      try:
         ...
         while(i<1000):
           ...
           if "atoms" in x:
             ...
             raise "Elk_converter : Could not find 'atoms' in GEOMETRY.OUT!"
           i+=1
        ...
        raise "Elk_converter : Could not open GEOMETRY.OUT!"
      R.close()
      return ns, na, atpos
    ...
    def read_bc(self):
        ...
        #no. of kpts and number of orbital
          ...
          if(gen['nspinor'] != nspinor):
            ...
            raise "Elk_converter (",file,") : reading nspinor failed!"
            return
          if(gen['natmtot'] != self.n_atoms):
            raise "Elk_converter (",file,") : reading no. of atoms failed!"
            return
          if(gen['nstsv'] != self.nstsv):
            raise "Elk_converter (",file,") : reading all states failed!"
            return
          if(gen['nkpt'] != self.n_k):
            raise "Elk_converter (",file,") : reading kpoints failed failed!"
            return
          if(gen['irep'] == 0):
            raise "Elk_converter (",file,") : Band characters are in spherical hamonics, may have issues with the PDOS!"
            return
          ...
          #check read in values
                if(ent['ispn'] != ispn+1):
                  raise "Elk_converter (",file,") : reading ispn failed!"
                  return
                if(ent['ias'] != iatom+1):
                  raise "Elk_converter (",file,") : reading iatom failed!"
                  return
                if(ent['ik'] != ik+1):
                  raise "Elk_converter (",file,") : reading ik failed!"
                  return

                for ist in range(self.nstsv):
                  for lm in range(dim):
                     bc[lm,ispn,iatom,ist,ik] = R.next()

        except StopIteration:  # a more explicit error if the file is corrupted.
            raise "Elk_converter (read BC.OUT): reading file failed!"
        R.close()
        return(bc,dim)
