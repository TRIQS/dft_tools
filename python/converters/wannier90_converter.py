...
class Wannier90Converter(ConverterTools):
    ...
    def convert_dft_input(self):
        ...
        try:
            ...
            #n_shells = int(R.next())            # total number of shells in the unit cell
        ...
        ### begin loop on isp
            ...
            if isp == 0:
                ...
                # check also on other quantities?

            hamr_full.append(hamr)
            #hamr_full.append(deepcopy(hamr)) # when do we actually need deepcopy()?
        ...
        iorb = 0
        # NOTE: we assume that the correlated orbitals appear at the beginning of the H(R) file
        # Therefore, the proj_mat has a ???
        for icrsh in range(n_corr_shells):
    ...
    def read_wannier90hr(self, hr_filename="wannier_hr.dat"):
        ...
        HofR = [numpy.zeros((num_wf, num_wf), dtype=numpy.complex_) for n in range(nrpt)]
            ...
            for ir in range(nrpt):
                ...
                for jj in range(num_wf):
                    for ii in range(num_wf):
                        ...
                        # fill HofR with the matrix elements of the Hamiltonian
                        HofR[ir][ii, jj] = complex(float(cline[5]), float(cline[6]))

        except ValueError:
            mpi.report("Wrong data or structure in file %s"%hr_filename)

        # return the data into variables
        return nrpt, rvec_idx, rvec_deg, num_wf, HofR
    ...
    def find_rot_mat(self, n_sh, sh_lst, sh_map, ham0):
        ...
        # TODO: handle case of degenerate eigenvalues
        eigval_lst = []
        eigvec_lst = []
        iwf = 0
        # loop over shells
        ...
        for ish in range(n_sh):
            try:
                ...
                mpi.report("Global-to-local rotations cannot be constructed!")
        ...
        # add additional check on matrices?
        # if not ? :
        #   istatus = 0
    ...
    def fourierham(self, norb, HofR):
        ...
        norb : integer
            number of orbitals
        HofR : list of numpy.array[norb,norb]
            Hamiltonian H(R) in Wannier basis

        Returns
        -------
        HofK : list of numpy.array[norb,norb]
        ...
        Hofk = [numpy.zeros((norb, norb), dtype=numpy.complex_) for ik in range(self.n_k)]
        for ik in range(self.n_k):
